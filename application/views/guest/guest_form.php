...
<head>
    ...
    <style>
        ...
        #keyboardKeys .shift-key {
            grid-column: span 2;
        }
        #keyboardKeys .enter-key {
            grid-column: span 2;
        }
    </style>
</head>
                    ...
                    <div class="position-relative mt-n7">
                        <div class="p-5 rounded-3 bg-body shadow-sm">
                            ...
                            <form id="voiceForm" class="needs-validation" method="POST" action="<?= site_url('GuestController/submit_form'); ?>" enctype="multipart/form-data">
                                ...
                                <p id="status" class="text-center mt-3">Status: Siap memulai, mohon tunggu...</p>
    ...
    <script>
        ...
        qwertyLayout.forEach(row => {
            row.forEach(key => {
                ...
                keyElement.className = `btn btn-outline-secondary me-1 mb-1 ${key === 'space' ? 'space-key' : key === 'backspace' ? 'backspace-key' : key === 'shift' ? 'shift-key' : key === 'enter' ? 'enter-key' : ''}`;
                keyElement.textContent = key === 'space' ? 'Spasi' : key === 'backspace' ? 'Hapus' : key === 'shift' ? 'Shift' : key === 'enter' ? 'Enter' : key;
                keyElement.onclick = () => {
                    ...
                    if (inputField.tagName === 'TEXTAREA' || inputField.type === 'text') {
                        if (key === 'space') {
                            ...
                            document.querySelectorAll('#keyboardKeys .shift-key').forEach(btn => {
                                btn.classList.toggle('btn-primary', isShiftActive);
                                btn.classList.toggle('btn-outline-secondary', !isShiftActive);
                            });
                        } else if (key === 'enter' && inputField.tagName === 'TEXTAREA') {
                            inputField.value += '\n';
                        } else if (key !== 'enter') {
                            inputField.value += isShiftActive ? key.toUpperCase() : key;
                        }
                    }
                };
        ...
        function startVoiceInput() {
            ...
            showCard(-1);
            const initialGuidance = 'Untuk mengisi formulir dengan suara, tunggu indikator di kanan atas berubah menjadi hijau. Anda boleh berbicara saat indikator hijau dan tidak boleh berbicara saat indikator merah.';
            document.getElementById('status').textContent = initialGuidance;
            speak(initialGuidance, () => {
                askCameraPermission();
            });
        }
        ...
        function askChatbotChoice() {
            ...
            const choiceText = 'Mohon menunggu konfirmasi dengan duduk di tempat yang telah disediakan. Apakah Anda ingin berbincang dengan chatbot untuk mengetahui lebih lanjut tentang SMKN 1 Subang? Ucapkan "ya" untuk chatbot, atau "tidak" untuk kembali ke halaman utama.';
        ...
        recognition.onresult = function(event) {
            ...
            if (isAskingCameraPermission) {
                ...
                if (transcript.includes('ya')) {
                    ...
                    document.getElementById('status').textContent = 'Baik, Anda akan diarahkan kembali ke halaman utama. Jika Anda ingin masuk ke chatbot, klik indikator warna merah di kanan atas.';
                    speak('Baik, Anda akan diarahkan kembali ke halaman utama. Jika Anda ingin masuk ke chatbot, klik indikator warna merah di kanan atas.', () => {
                        Swal.close();
